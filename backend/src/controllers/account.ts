...
export const googleLogin = async (req: Request, res: Response) => {
  try {
    ...
    // si existe pero fue creado con otro provider
    if (account && account.provider !== "google") {
      return res.status(400).json({
        status: "error",
        message: "This account is already registered with another method.",
      });
    }
...
export const facebookLogin = async (req: Request, res: Response) => {
  try {
    ...
    if (!email) {
      return res.status(400).json({
        status: "error",
        message: "Facebook did not provide an email",
      });
    }

    let account = await Account.findOne({ email });

    // si existe pero con otro provider
...
export const getAllAccounts = async (req: AuthRequest, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ status: "error", message: "Error getting accounts" });
  }
};
...
export const updateAccount = async (req: AuthRequest, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ status: "error", message: "Error updating account" });
  }
};
...
export const deleteAccount = async (req: AuthRequest, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ status: "error", message: "Error deleting account" });
  }
};
